...
//constexpr static float s_ballUpdate=0.2;
...
void NGLScene::reset()
{
    ...
    m_ball->set(ngl::Vec3 (0.0f, 0.0f, 0.0f), ngl::Vec3 (0.0f, 0.0f, 0.0f), 5.0f);
    m_paddle.reset();
    m_paddle2.reset();
    m_ball.reset();
}
...
void NGLScene::initializeGL()
{
  ...
  ngl::Light light1(ngl::Vec3( 0, 0, 2),ngl::Colour( 1, 0, 1, 1),ngl::Colour( 1, 0, 1, 1),ngl::LightModes::SPOTLIGHT);
  //ngl::Light light2(ngl::Vec3(0,5,0),ngl::Colour(0,0,1,1),ngl::Colour(0,0,1,1),ngl::LightModes::DIRECTIONALLIGHT);
  ...
  //m_plane.reset( new Plane(ngl::Vec3(m_win.width/2, m_win.height/2, 0), "models/plane.obj"));
  ...
  //create out paddles
  m_paddle.reset( new Paddle(ngl::Vec3(0, 20, 0), "models/paddle3.obj"));
  m_paddle2.reset( new Paddle(ngl::Vec3(0, -20, 0), "models/paddle3.obj"));
  ...
  m_text.reset( new ngl::Text(QFont("Arial",20)));
...
void NGLScene::paintGL()
{
  ...
  QString text = QString("                                            SCORE: %d : %d");
  ...
  //m_plane->draw("Phong", &m_cam);

  m_paddle->draw("Phong", &m_cam);
  m_paddle2->draw("Phong", &m_cam);
...
void NGLScene::checkCollisions()
{
  ...
  ngl::Vec3 paddlePos2 = m_paddle2->getPos();
  ngl::Vec3 paddlePos = m_paddle->getPos();
  ngl::Vec3 ballPos = m_ball->getPos();

  float range = 4.0f;

  if ( abs( paddlePos.m_x - ballPos.m_x ) < range )
    if ( abs( paddlePos.m_y - ballPos.m_y ) < range )
    {
      std::cout << "x: " << paddlePos.m_x << " - " << ballPos.m_x << "\n";
      std::cout << "y: " << paddlePos.m_y << " - " << ballPos.m_y << "\n\n";
  ...
  if ( abs( paddlePos2.m_x - ballPos.m_x ) < range )
    if ( abs( paddlePos2.m_y - ballPos.m_y ) < range )
    {
      std::cout << "x: " << paddlePos.m_x << " - " << ballPos.m_x << "\n";
      std::cout << "y: " << paddlePos.m_y << " - " << ballPos.m_y << "\n\n";
...
void NGLScene::movePaddle()
{
  ...
  {
    m_paddle->move(xDirection,yDirection);
  }
}

void NGLScene::movePaddle2()
{
  /// first we reset the movement values
  float xDirection=0.0;
  float yDirection=0.0;
  ...
  if(m_keysPressed.size() !=0)
  {
    m_paddle2->move(xDirection, yDirection);
  }
}

void NGLScene::GUI()
{
  //Need to include Qimage for the title screen!!
  //Pressing enter starts the game, and escape will quit the game.
  foreach(Qt::Key key, m_keysPressed)
  {
    switch (key)
    {
